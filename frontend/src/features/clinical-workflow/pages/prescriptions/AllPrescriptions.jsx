...
function AllPrescriptions({ search = '' }) {
  ...
  // Helper function to check if prescription date is today
  const isPrescriptionDateToday = (prescriptionDate) => {
    ...
    return today.getTime() === prescDate.getTime();
  };

  return (
    <div className="all-prescriptions-wrapper">
      <div className="d-flex justify-content-between align-items-center mb-4 no-print">
            <div className="ap-enhanced-title-wrapper">
              <div className="ap-title-badge">📋</div>
              <div className="ap-title-content">
                <h1 className="ap-enhanced-title mb-0">All Prescriptions</h1>
                <p className="ap-title-subtitle mb-0">Comprehensive Medical Prescription Records</p>
              </div>
            </div>
        ...
        {filtered.length > 0 ? filtered.map(p => (
          <div key={p._id} className="ap-tab-card show-all">
            ...
            <div className="ap-actions mt-3 no-print" style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 24 }}>
              {isPrescriptionDateToday(p.Date) ? (
                <Link to={`/prescription/update/${p._id}`} className="btn-ap-update me-2">Update</Link>
              ) : (
                <button 
                  className="btn-ap-update me-2" 
                  style={{ 
                    opacity: 0.5, 
                    cursor: 'not-allowed',
                    pointerEvents: 'none',
                  ...
                  disabled
                  title="Can only update prescriptions from today"
                >
                  Update
                </button>
              )}
              <Link to={`/prescription/delete/${p._id}`} className="btn-ap-delete">Delete</Link>
            </div>
          </div>
        )) : (
