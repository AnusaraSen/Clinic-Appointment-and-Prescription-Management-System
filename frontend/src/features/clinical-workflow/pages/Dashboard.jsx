...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const [statsRes, appointmentsRes, tasksRes, activitiesRes, patientsRes] = await Promise.all([
        ...
        axios.get(`${base}/patients?limit=1`) // Get patients with minimal limit to just get the total count
      ]);

      setStats({
        ...statsRes.data,
        totalMedicalRecords: patientsRes.data.total || 0
      });
      ...
      // If patients endpoint fails, still show other data
      if (error.response?.config?.url?.includes('/patients')) {
        console.warn('Failed to fetch patient count, using fallback');
        setStats(prev => ({ ...prev, totalMedicalRecords: 0 }));
      }
    } finally {
      setLoading(false);
    }
        ...
        <div className="stats-grid">
          ...
          <div className="stat-card">
            ...
            <div className="stat-content">
              ...
              <div className="stat-change">All records secure</div>
            </div>
          </div>
